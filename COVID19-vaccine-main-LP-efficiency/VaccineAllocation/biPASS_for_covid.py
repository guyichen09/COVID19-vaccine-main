# Here we assume that each system has the same run length function

# Misc LP WIP notes

# Test with known and estimated variances
# Debugging case and test

# For simulation model, need to read-in the .json file of feasible solutions

# Master main loop function?
# Worker main loop function?
# Master "calibration" function?
# Worker "calibration" function?
# "Calibration" -- types of scaling factors? Known variance, custom scaling, pooled scaling

# Standard update
# Simulation model
# Boundary function

# Need to change SEIR randomgen
	...
	if rank == master_rank:
		...
		while cycle < config.num_cycles and len(contenders) > 1:
			...
			while num_pending_messages > 0:
				...
				if cycle == 1 and config.est_variances == False:
