...
- Edge case for tiers at the end of the historical period (start of the optimization horizon): the original version of the code computes tiers for the historical period. Won't this affect the optimization period in some cases? Because we have the 14-day rule!!!!
- Need to update ```calendar.csv``` if simulation is run past 09/30/2022 ya? Is there a source for this data?
- In ```setup_data_Final.json```, why is ```new_variant``` set to ```False``` ?
- OtherInfo stuff???? What is that? Ask Nazlican about the overwriting. What is ```update_icu_params``` and ```kwargs["rd_rate"]``` and all that? And what's the difference between this method and ```update_icu_all?```

# 08162022 Question Draft

As an addendum and more context to question #1, the current code has a structure like this...
...
For each timepoint t:
   *do stuff*
   if t >= delta_start:
       delta_update_param
   if t >= omicron_start:
       omicron_update_param

I tested the following variation of the code:

For each timepoint t:
   *do stuff*
   if t >= delta_start and t < omicron_start:
       delta_update_param
   if t >= omicron_start:
       omicron_update_param

^ In the variation, we only do delta_update_param 

THE RESULTS ARE DIFFERENT

At time 700, self.epi.gamma_ICU, self.epi.mu_ICU, self.epi.gamma_IH are all different!

# 08112022 Question

My question: we supposed to be updating epidemiological variables (attributes in the instance of ```EpiSetup```) recursively or with respect to original values? Thank you ahead of time for reading and for your help. I am potentially confused and I am wondeirng if you can help me with some questions.

By updating variables recursively, I mean that every time a variable $x$ is updated, the update looks something like this:
\[x = x + \texttt{some stuff},\]
so that $x$ depends on its previous value when it was last updated.

By updating variables with respect to original values, I mean that every time a variable $x$ is updated, the update looks something like this:
\[x = x_0 + \texttt{some stuff},\]
so that $x$ does \textit{not} depend on its previous value, and instead only depends on some "nominal" or "ordinal" value $x_0$ that does not change throughout successive updates.
...
Below are selected important lines from the function ```simulate_t``` which is called for every day $t = 0, 1, 2, \dots, T$. This is not a verbatim copy and paste, as I have omitted some lines for clarity.

Below, ```epi_rand``` is an instance of ```EpiSetup```, which contains important epidemiological variables such as transmissibility, hospital stay, etc... Because every time we call ```simulate_t``` (i.e. every day we simulate), we work on a new \textit{copy} of ```epi_rand```, \textit{the first time that we update an epidemiological variable within this day, this update is with respect to original values. This update does not recursively depend on the previous day's values.} 

However, because these updates are done "in-place" by modifying ```epi``` attributes, the second time that we update an epidemiological variable within this day, this update is \textit{recursive}.

Consider these line excerpts from ```delta_update_param``` and ```omicron_update_param``` (both are instance methods of ```EpiSetup``` objects).
...
```{}
    ...
    def omicron_update_param(self, prev):
        ...
        self.beta = self.beta * (1 - prev) + self.beta * (self.omicron_beta) * prev 
...
If we are at a day $t$ that is after both the Delta variant start and the Omicron variant start, then every day, we call both above functions. 

I have the following specific concerns:

1. There seems to be an inconsistency: ```delta_update_param``` updates ```beta``` with respect to the original ```beta0``` whereas ```omicron_update_param``` updates ```beta``` recursively. The opposite happens with ```YHR.```

2. When we do the Omicron updating, ```beta``` refers to a previous value of ```beta``` that is \textit{not} the original value, but rather the \textit{value edited by the Delta updating}. 

3. When we do the Omicron updating, because we update ```YHR``` with respect to the original ```YHR0```, this totally erases the impact of Delta. Therefore, the ```YHR``` value is as if only Omicron exists but Delta does not exist. Is this a bug or an assumption that we are purposely making about the model? 

In summary, are these parameters (and other parameters) being updated correctly? Is there another way we should be combining the impacts of Delta and Omicron? A similar issue happens when there is a new variant of concern and we update ```self.beta```. Thank you for the help and clarification!
...
- Potential bug regarding omicron update (I emailed Nazlican to ask)
  - See ```SEIYAHRD.py```'s ```simulate_t``` function
  ```{}
